...
static int hmac_sha256_selftest(void)
{
    ...
    if (memcmp(digest, (uint8_t[SHA256_DIGESTSZ]){ 0x77,0x3e,0xa9,0x1e,0x36,0x80,0x0e,0x46,0x85,0x4d,0xb8,0xeb,0xd0,0x91,0x81,0xa7, 0x29,0x59,0x09,0x8b,0x3e,0xf8,0xc1,0x22,0xd9,0x63,0x55,0x14,0xce,0xd5,0x65,0xfe }, SHA256_DIGESTSZ))
        return 1;

    memcpy(t1, "Jefe", 4);
    memcpy(d1, "what do ya want for nothing?", 28);
    HMAC_SHA256(digest, t1, 4, d1, 28);
    if (memcmp(digest, (uint8_t[SHA256_DIGESTSZ]){ 0x5b,0xdc,0xc1,0x46,0xbf,0x60,0x75,0x4e,0x6a,0x04,0x24,0x26,0x08,0x95,0x75,0xc7, 0x5a,0x00,0x3f,0x08,0x9d,0x27,0x39,0x83,0x9d,0xec,0x58,0xb9,0x64,0xec,0x38,0x43 }, SHA256_DIGESTSZ))
        return 1;
    return 0;
}
