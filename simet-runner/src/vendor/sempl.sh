...
__sempl_template=''
__sempl_outfile=''
# simet edit: comment as ash shell doesn't know arrays
# __sempl_varsfiles=( )
__sempl_password=''
...
_convert_template() {
  ...
  # simet edit: quote variable for ash
  # while IFS='' read __line || [[ -n ${__line} ]]; do
  while IFS='' read __line || [[ -n "$__line" ]]; do
    # simet edit: openwrt not matching regex (\s) using [[:blank:]] instead
    # if echo "${__line}" | grep -E -q '^(\s+)?###(\s)?begin'; then
    if echo "${__line}" | grep -E -q '^[[:blank:]]*###[[:blank:]]?begin'; then
      __in_block=1
      __tmp_script=$(mktemp -t sempl.XXXXXX)
      continue
    fi
    # if echo "${__line}" | grep -E -q '^(\s+)?###(\s)?end'; then
    if echo "${__line}" | grep -E -q '^[[:blank:]]*###[[:blank:]]?end'; then
      __in_block=0
      # simet edit: sh instead of bash, for busybox compatibility
      #bash $__tmp_script >> $__tmp_tmpl
      sh $__tmp_script >> $__tmp_tmpl
      rm $__tmp_script
      continue
    fi
    if [ $__in_block -eq 1 ]; then
      if echo "${__line}" | grep -E -q '^[[:blank:]]*#'; then
...
# simet edit: rename for unique name, when concatenating scripts during build
#_main() {
_sempl() {
  [ -z "$1" ] && { _usage; exit 1; }
